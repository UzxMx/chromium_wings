...
WingsDevToolsBindings::WingsDevToolsBindings(content::WebContents* devtools_contents,
  ...
  : content::WebContentsObserver(devtools_contents),
    ...
    weak_factory_(this) {
  LOG(INFO) << inspect_element_at_x_ << inspect_element_at_y_ << (delegate_ != nullptr);
}

WingsDevToolsBindings::~WingsDevToolsBindings() {
...
void WingsDevToolsBindings::InspectElementAt(int x, int y) {
  LOG(INFO) << "unimplemented";
}

void WingsDevToolsBindings::Attach() {
  if (agent_host_)
    agent_host_->DetachClient(this);
...
void WingsDevToolsBindings::CallClientFunction(const std::string& function_name,
...
void WingsDevToolsBindings::AgentHostClosed(content::DevToolsAgentHost* agent_host) {
  LOG(INFO) << "unimplemented";
}

void WingsDevToolsBindings::DispatchProtocolMessage(content::DevToolsAgentHost* agent_host,
...
void WingsDevToolsBindings::HandleMessageFromDevToolsFrontend(const std::string& message) {
  std::string method;
  ...
  if (method == "dispatchProtocolMessage" && params && params->GetSize() == 1) {
    ...
    if (!agent_host_ || !params->GetString(0, &protocol_message))
      return;
    LOG(INFO) << "To agent: " << protocol_message;
...
void WingsDevToolsBindings::ReadyToCommitNavigation(content::NavigationHandle* navigation_handle) {
  content::RenderFrameHost* frame = navigation_handle->GetRenderFrameHost();
  if (navigation_handle->IsInMainFrame()) {
    frontend_host_.reset(content::DevToolsFrontendHost::Create(
        frame,
        base::Bind(&WingsDevToolsBindings::HandleMessageFromDevToolsFrontend,
...
void WingsDevToolsBindings::WebContentsDestroyed() {
  LOG(INFO) << "unimplemented"; 
}

// net::URLFetcherDelegate overrides.
void WingsDevToolsBindings::OnURLFetchComplete(const net::URLFetcher* source) {
  LOG(INFO) << "unimplemented"; 
}

void WingsDevToolsBindings::SendMessageAck(int request_id, const base::Value* arg1) {
